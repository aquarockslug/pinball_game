class Pinball extends Phaser.Scene {
	...
	preload() {
		...
		[['sky', 'sky.png'], ['board', 'board.png'], ['ball', 'silver_ball.png'], ['bumper', 'bumper_oval.png'], 
		['shade', 'shade.png'], ['wall', 'wall.png'], ['paddleLeft', 'paddleLeft.png'], ['paddleRight', 'paddleRight.png'],
	...
	create() {
		...
		this.add.text(900, 75, 'Score: 0', { fontSize: '24px' });
	}
	...
	createSlingshots () {
		...
		this.add.image(pos.paddle.x+spread/2, pos.paddle.y-100, 'slingshot', 
			{ shape: spritePhysics['slingshot'] }).setScale(0.25)
		this.add.image(pos.paddle.x-spread/2, pos.paddle.y-100, 'slingshot', 
			{ shape: spritePhysics['slingshot'] }).setScale(0.25).setFlipX(true)
	}
	...
	createWalls () {
		...
		this.matter.add.rectangle(pos.board.x, 40, 400, 30, { isStatic: true })
		...
		this.matter.add.sprite(pos.board.x + 200, pos.board.y-25, 
			'wall', { shape: spritePhysics['wall'] }).setScale(1.0, 0.2).setStatic(true).setAngle(80)
	}
...
pos = {
	center: { x: w / 2, y: h / 2 },
	launch: { x: 670, y:  600},
	paddle: { x: 370, y: h / 2 + 100 },
	board: { x: 385, y: h / 2}
}

function newBall(scene) {
	const ball = scene.matter.add.image(pos.launch.x, pos.launch.y, 'ball', null, { label: 'ball'});
...
function newPaddles(scene, center = pos.paddle) {
	...
	const options = { spread: 250, matter: {} }
